...
import org.apache.spark.mllib.fpm.{DIAssociationRules => MLlibAssociationRules, FPGrowth => MLlibFPGrowth}
...
private[fpm] trait FPGrowthParams extends Params with HasPredictionCol {
  ...
  protected def validateAndTransformSchema(schema: StructType): StructType = {
    ...
    require(inputType.isInstanceOf[ArrayType],
      s"The input column must be ${ArrayType.toString}, but got ${inputType.catalogString}.")
