 ...
 */
@Since("1.3.0")
class DIFPGrowthModel[Item: ClassTag] @Since("2.4.0") (
   ...
   */
  @Since("1.5.0")
  def generateAssociationRules(confidence: Double): RDD[DIAssociationRules.Rule[Item]] = {
    val associationRules = new DIAssociationRules(confidence)
   ...
   *
   * The model may be loaded using `DIFPGrowthModel.load`.
   *
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    DIFPGrowthModel.SaveLoadV1_0.save(this, path)
  }
...
@Since("2.0.0")
object DIFPGrowthModel extends Loader[DIFPGrowthModel[_]] {

  @Since("2.0.0")
  override def load(sc: SparkContext, path: String): DIFPGrowthModel[_] = {
    DIFPGrowthModel.SaveLoadV1_0.load(sc, path)
  }
  ...
  private[fpm] object SaveLoadV1_0 {
    ...
    private val thisClassName = "org.apache.spark.mllib.fpm.DIFPGrowthModel"

    def save(model: DIFPGrowthModel[_], path: String): Unit = {
    ...
    def load(sc: SparkContext, path: String): DIFPGrowthModel[_] = {
    ...
    def loadImpl[Item: ClassTag](freqItemsets: DataFrame, sample: Item): DIFPGrowthModel[Item] = {
      ...
      new DIFPGrowthModel(freqItemsetsRDD)
    }
  }
}
...
class FPGrowth private[spark] (
   ...
   * @return an [[DIFPGrowthModel]]
   *
   */
  @Since("1.3.0")
  def run[Item: ClassTag](data: RDD[Array[Item]]): DIFPGrowthModel[Item] = {
    ...
    new DIFPGrowthModel(freqItemsets, itemSupport)
  }
   ...
   */
  @Since("1.3.0")
  def run[Item, Basket <: JavaIterable[Item]](data: JavaRDD[Basket]): DIFPGrowthModel[Item] = {
